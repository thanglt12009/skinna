...
namespace SkinnaManagement.WebPages.QuanLyDonHang
{
    public partial class EditDonHang : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!this.IsPostBack)
            {
                ...
                if(donHang != null)
                {
                    ...
                    TongTien.Text = donHang.TongTienDonHang.ToString();

                }
                SetInitialRow(donHang.MaDonHang);
            }
        }
        ...
        private void SetInitialRow(int maDonHang)
        {
            ...
            if (list != null && list.Count > 0)
            {
                foreach (var donhang in list)
                {
                    ...
                    dr["Column2"] = sanPham.TenSanPham;
                    dr["Column3"] = donhang.DonGia.ToString();
                    dr["Column4"] = donhang.SoLuong.ToString();
                    dr["Column5"] = donhang.ThanhTien.ToString();
                    dt.Rows.Add(dr);
                    i++;
                }
            }
            dr = dt.NewRow();
            dr["RowNumber"] = list.Count + 1;
            dr["Column1"] = string.Empty;
            ...
            gvProducts.DataSource = dt;
            gvProducts.DataBind();            
            DropDownList product = (DropDownList)gvProducts.Rows[list.Count].Cells[2].FindControl("SanPham");
            Label DonGia = (Label)gvProducts.Rows[list.Count].Cells[3].FindControl("DonGia");
            Label ThanhTien = (Label)gvProducts.Rows[list.Count].Cells[5].FindControl("ThanhTien");
            TextBox SoLuong = (TextBox)gvProducts.Rows[list.Count].Cells[4].FindControl("SoLuong");
            TList<KhoHangSanPham> sanPhamList = DataRepository.KhoHangSanPhamProvider.GetAll();
            if (sanPhamList != null && sanPhamList.Count > 0)
            {
                //Building an HTML string.
            ...
            SoLuong.Text = "1";
            for (int j = 0; j < gvProducts.Rows.Count - 1; j++)
            {
                product = (DropDownList)gvProducts.Rows[j].Cells[2].FindControl("SanPham");
                DonGia = (Label)gvProducts.Rows[j].Cells[3].FindControl("DonGia");
                ThanhTien = (Label)gvProducts.Rows[j].Cells[5].FindControl("ThanhTien");
                SoLuong = (TextBox)gvProducts.Rows[j].Cells[4].FindControl("SoLuong");
                if (sanPhamList != null && sanPhamList.Count > 0)
                {
                    //Building an HTML string.
                    StringBuilder html = new StringBuilder();
                ...
                DonHangChiTiet donHang = list[j];
                sanpham = DataRepository.KhoHangSanPhamProvider.GetByMaSanPham(donHang.MaSanPham.GetValueOrDefault(-1));
                if (sanpham != null)
                {
